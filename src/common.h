...
namespace QGit {
	...
	const int FLAGS_DEF = SMART_LBL_TB_F | USE_CMT_MSG_F | RANGE_SELECT_F | SMART_LBL_F | VERIFY_CMT_F | SIGN_PATCH_F | LOG_DIFF_TAB_F | MSG_ON_NEW_F;
...
public:
	...
	const QString committer() const { setup(); return mid(comStart, comDateStart - comStart - 1); }
...
private:
	...
	const int start;
	mutable int parentsCnt, shaStart, autStart, autDateStart, comStart, comDateStart;
